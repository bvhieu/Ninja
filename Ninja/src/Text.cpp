...
void Text::Draw(SDL_Renderer* renderer, std::string sText, int X, int Y, int fontSize) {
	...
	for(unsigned int i = 0; i < sText.size(); i++) {
		...
		rRect.w = fontSize;
		rRect.h = fontSize;
		FONT->Draw(renderer, rCrop, rRect);
		extraLeft += nextExtraLeft;
		nextExtraLeft = 0;
	}
}

void Text::Draw(SDL_Renderer* renderer, std::string sText, int X, int Y, int fontSize, int iR, int iG, int iB) {
	this->fontSize = fontSize;
	this->extraLeft = 0;
	this->nextExtraLeft = 0;
...
void Text::DrawCenterX(SDL_Renderer* renderer, std::string sText, int Y, int fontSize, int iR, int iG, int iB) {
	int X = CommonFunctions::GAME_WIDTH / 2 - getTextWidth(sText, fontSize) / 2;

	Draw(renderer, sText, X, Y, fontSize, iR, iG, iB);
}

void Text::Draw(SDL_Renderer* renderer, std::string sText, int X, int Y, int iWidth, int iHeight) {
	for(unsigned int i = 0; i < sText.size(); i++) {
		rCrop.x = getPos(sText.at(i));
...
void Text::DrawWS(SDL_Renderer* renderer, std::string sText, int X, int Y,int iR, int iG, int iB, int fontSize) {
	this->fontSize = fontSize;
	this->extraLeft = 0;
	this->nextExtraLeft = 0;
...
void Text::setFont(SDL_Renderer* renderer, std::string fileName) {
	FONT = new Image(fileName, renderer);
}